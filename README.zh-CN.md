...
下面这些目标已经列入后续计划，但目前还没有实现：

- 优化 Agent，把过程精简输出到前端，同时让用户始终有参与感。
- 优化 DSL 语法，让解释器更精准地表达用户需求，帮助渲染引擎更好理解场景。
- 理清 Agent 工作流，收束场景和反馈问答，让工程多干活、少问话。
- 增加前端操作能力，例如拖拽镜头、放大缩小，并适配 Windows 和 macOS 的快捷键。
- 打通组件与渲染引擎的联动，支持手动编辑组件参数并实时反映到场景中。
